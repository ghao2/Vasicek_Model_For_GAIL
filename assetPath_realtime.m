...
% some properties particulary for this class.
   ...
   properties (SetAccess=public) %so they can only be set by the constructor
      assetParam = struct('pathType','GBM', ... %type of asset path
         ...
         'alpha',0.1,...%alpha, the stregnth of pull towards the nomial value
         'intVolatility',0.5,... %the volatility of interest
         'drift', 0,... %drift
   ...
   properties (Constant, Hidden) %do not change & not seen
      allowPathType = {'GBM','QE_m','QE','VCEK'} 
...
% the asset paths based on |whiteNoise| paths.
   ...
   methods
      ...
      function obj = assetPath(varargin)         
         ...
         if nargin>0
            ...
            if isfield(obj.restInput,'assetParam')
               ...
               obj.assetParam = val; %this will triger the function set.assetParam(obj, val)
      ...
      function set.assetParam(obj,val)
         ...
         if isfield(val,'alpha') %data for type of option
            validateattributes(val.alpha,{'numeric'},...
                {'nonnegative'})
            obj.assetParam.alpha=val.alpha(:); 
         end
         
         if isfield(val,'intVolatility') %data for type of option
            validateattributes(val.intVolatility,{'numeric'}, ...
               {'nonnegative'})
           if numel(val.intVolatility) == obj.assetParam.nAsset
            obj.assetParam.intVolatility=val.intVolatility(:); %row
           else
                obj.assetParam.intVolatility ...
                    =repmat(val.intVolatility(1),obj.assetParam.nAsset,1);
           end
         end
      ...
      function [paths]=genPaths(obj,val)
         bmpaths = genPaths@brownianMotion(obj,val);  %here is very important
                                                      %this step 
                                                      %firstly
                                                      %generate paths from
                                                      %whiteNoise, it is a
                                                      %matrix of rand
                                                      %humbers(distribution depending on the input wnParam.xDistrib)
                                                      %Secondly
                                                      %the paths
                                                      %were passed to
                                                      %brownianMotion to
                                                      %generate
                                                      %brownianMotion paths
                                                      %and return to
                                                      %bmpaths
                                                      
                                                      
         nPaths = size(bmpaths,1);             %return the length of first dimension, meaning how many
                                                %rows are there in first
                                                %column
                                                
         if strcmp(obj.assetParam.pathType,'GBM')    %if path type is GBM
             

            tempc=zeros(nPaths,obj.timeDim.nSteps);  %create 0 matrix with the same dimension of paths matrix
            paths=zeros(nPaths,obj.timeDim.nCols);   %nCols=nSteps*dim
            for idx=1:obj.assetParam.nAsset
              colRange = ...
                 ((idx-1)*obj.timeDim.nSteps+1):idx*obj.timeDim.nSteps;
             
             %the for loop below is to initialize the Xj matrix in formula
             %in the case of dim==1, tempc is the brownie motion paths that
             %is returned from brownianMotion(obj,val), and sqCorr is 1.
              for j=1:obj.timeDim.nSteps
                 tempc(:,j)=bmpaths(:,j:obj.timeDim.nSteps:obj.timeDim.nCols) ...
                    * obj.sqCorr(:,idx);
              end
              
              %this code below is the geometric brownie motion formula
              %so, in order to implement VCEK model, we only have to modify
              %the 'interest'
              paths(:,colRange) = obj.assetParam.initPrice(idx) * ...
                 ...
                 .* tempc));
             %timeVector is a row vector, it is added to each row of tempc
             %by bsxfun() function
         ...
         %VCEK model
         if strcmp(obj.assetParam.pathType,'VCEK')    %if path type is VCEK
            %firstly, realize that the interest rate should be a vector
            
            %1, generate the brownie motion Xj for interest rate, we only need one path
            intPaths = genPaths@brownianMotion(obj,1);
            
            %2, generate interest rate vector
            r = zeros(1, obj.timeDim.nSteps);
            
            %3, initialize the first element with r0
            r0=obj.assetParam.interest;
            
            %4, calculate r(1) with the formula
            r(1)=r0+obj.assetParam.intVolatility...
                *sqrt((1-exp(-2*obj.assetParam.alpha*obj.timeDim.timeIncrement(1)))/(2*obj.assetParam.alpha))...
                *intPaths(1);
            
            %5, calculate the entire path iteratively
            for j=2:obj.timeDim.nSteps
                r(j)=exp(-obj.assetParam.alpha*obj.timeDim.timeIncrement(1))*r(j-1)...
                    +(1-exp(-obj.assetParam.alpha*obj.timeDim.timeIncrement(1)))*r0...
                    +obj.assetParam.intVolatility...
                    *sqrt((1-exp(-2*obj.assetParam.alpha*obj.timeDim.timeIncrement(1)))/(2*obj.assetParam.alpha))...
                    *intPaths(j);
            ...
            %secondly, generate the VCEK paths 
            tempc=zeros(nPaths,obj.timeDim.nSteps);  %create 0 matrix with the same dimension of paths matrix
            paths=zeros(nPaths,obj.timeDim.nCols);   %nCols=nSteps*dim
            for idx=1:obj.assetParam.nAsset
              colRange = ...
...
%change Ntime to timeDim.nSteps ***********************************%    
             ...
             dW2 = [bmpaths(:,1) diff(bmpaths(:,1:Ntime),1,2)]/sqrt(dT); %Use Brownian motion to generate normal distribution
